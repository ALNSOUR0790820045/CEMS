...
<div style="padding: 40px; max-width: 1000px; margin: 0 auto;">
    <h1 style="margin-bottom: 30px;">إضافة عنصر WBS جديد</h1>
    <p style="color: #86868b; margin-bottom: 30px;">{{ $tender->name }} - {{ $tender->reference_number }}</p>
    
    <form method="POST" action="{{ route('tender-wbs.store', $tender->id) }}" style="background: white; padding: 30px; border-radius: 10px;">
        @csrf
        ...
        <div style="border-bottom: 2px solid #f0f0f0; padding-bottom: 20px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    ...
                    <input type="text" name="wbs_code" value="{{ old('wbs_code') }}" required 
                ...
                <div>
                    ...
                    <select name="level" required 
                        ...
                        <option value="1" {{ old('level') == 1 ? 'selected' : '' }}>المستوى 1</option>
                        <option value="2" {{ old('level') == 2 ? 'selected' : '' }}>المستوى 2</option>
                        <option value="3" {{ old('level') == 3 ? 'selected' : '' }}>المستوى 3</option>
                        <option value="4" {{ old('level') == 4 ? 'selected' : '' }}>المستوى 4</option>
                        <option value="5" {{ old('level') == 5 ? 'selected' : '' }}>المستوى 5</option>
                    </select>
                    @error('level')
                        <span style="color: #ff3b30; font-size: 0.85rem;">{{ $message }}</span>
                    @enderror
            ...
            <div style="margin-bottom: 20px;">
                <label>
                    <input type="checkbox" name="is_summary" value="1" {{ old('is_summary') ? 'checked' : '' }}>
        ...
        <div style="border-bottom: 2px solid #f0f0f0; padding-bottom: 20px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="number" name="materials_cost" value="{{ old('materials_cost', 0) }}" step="0.01" min="0"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                </div>
                
                <div>
                    <label style="display: block; margin-bottom: 5px; font-weight: 600;">تكلفة العمالة</label>
                    <input type="number" name="labor_cost" value="{{ old('labor_cost', 0) }}" step="0.01" min="0"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                </div>
                
                <div>
                    <label style="display: block; margin-bottom: 5px; font-weight: 600;">تكلفة المعدات</label>
                    <input type="number" name="equipment_cost" value="{{ old('equipment_cost', 0) }}" step="0.01" min="0"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                </div>
                
                <div>
                    <label style="display: block; margin-bottom: 5px; font-weight: 600;">تكلفة مقاولي الباطن</label>
                    <input type="number" name="subcontractor_cost" value="{{ old('subcontractor_cost', 0) }}" step="0.01" min="0"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                </div>
        ...
        <div style="border-bottom: 2px solid #f0f0f0; padding-bottom: 20px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <input type="number" name="estimated_duration_days" value="{{ old('estimated_duration_days') }}" min="0"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                </div>
                
                <div>
                    <label style="display: block; margin-bottom: 5px; font-weight: 600;">الوزن النسبي (%)</label>
                    <input type="number" name="weight_percentage" value="{{ old('weight_percentage', 0) }}" step="0.01" min="0" max="100"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                </div>
