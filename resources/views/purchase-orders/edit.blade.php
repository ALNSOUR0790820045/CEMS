...
<div class="page-header">
    <h1 class="page-title">تعديل أمر شراء - {{ $purchaseOrder->order_number }}</h1>
</div>
    ...
    <div class="form-card">
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="date" name="expected_date" class="form-control" value="{{ old('expected_date', $purchaseOrder->expected_date?->format('Y-m-d')) }}">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="payment_term_id" class="form-select" required>
                    ...
                    @foreach($paymentTerms as $term)
                        <option value="{{ $term->id }}" {{ (old('payment_term_id', $purchaseOrder->payment_term_id) == $term->id) ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="discount" class="form-control" value="{{ old('discount', $purchaseOrder->discount) }}" id="orderDiscount">
            </div>
        </div>
...
<script>
    ...
    function addItem(existingItem = null) {
        ...
        const taxRate = existingItem ? existingItem.tax_rate : 15;
        ...
        row.innerHTML = `
            <td>
                <select name="items[${itemCount}][product_id]" class="item-input" required onchange="updateProductPrice(${itemCount})">
                    ...
                    ${products.map(p => `<option value="${p.id}" data-price="${p.cost_price}" data-tax="${p.tax_rate}" ${p.id == productId ? 'selected' : ''}>${p.name} (${p.sku})</option>`).join('')}
                </select>
            </td>
            <td>
                <input type="number" step="0.001" name="items[${itemCount}][quantity]" class="item-input" value="${quantity}" min="0.001" required onchange="calculateItem(${itemCount})">
            </td>
            <td>
                <input type="number" step="0.01" name="items[${itemCount}][unit_price]" class="item-input" value="${unitPrice}" min="0" required onchange="calculateItem(${itemCount})">
            </td>
            <td>
                <input type="number" step="0.01" name="items[${itemCount}][tax_rate]" class="item-input" value="${taxRate}" min="0" max="100" onchange="calculateItem(${itemCount})">
            </td>
            <td>
                <input type="number" step="0.01" name="items[${itemCount}][discount]" class="item-input" value="${discount}" min="0" onchange="calculateItem(${itemCount})">
            </td>
            <td>
    ...
    if (existingItems && existingItems.length > 0) {
        ...
        // Add first empty item if creating new
        addItem();
    }
