...
<div style="margin-bottom: 30px;">
    <h1 style="font-size: 1.8rem; font-weight: 700; color: #1d1d1f; margin-bottom: 10px;">تعديل أمر التغيير {{ $changeOrder->co_number }}</h1>
    <p style="color: #666;">قم بتعديل الحقول المطلوبة</p>
</div>

<form method="POST" action="{{ route('change-orders.update', $changeOrder) }}" enctype="multipart/form-data" id="coForm">
    @csrf
    @method('PUT')
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>رقم CO *</label>
                    <input type="text" value="{{ $changeOrder->co_number }}" readonly class="calculated-field">
                </div>
                <div class="form-group">
                    <label>التاريخ *</label>
                    <input type="date" name="issue_date" value="{{ old('issue_date', $changeOrder->issue_date->format('Y-m-d')) }}" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="project_id" required id="project_id">
                        ...
                        <option value="{{ $project->id }}" {{ $project->id == $changeOrder->project_id ? 'selected' : '' }}>{{ $project->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="form-group">
                <label>العنوان *</label>
                <input type="text" name="title" value="{{ old('title', $changeOrder->title) }}" required placeholder="عنوان أمر التغيير">
            </div>

            <div class="form-group">
                <label>الوصف التفصيلي *</label>
                <textarea name="description" required placeholder="اشرح التغييرات المطلوبة بالتفصيل">{{ old('description', $changeOrder->description) }}</textarea>
            </div>

            <div class="form-group">
                <label>التبرير</label>
                <textarea name="justification" placeholder="أسباب وتبريرات التغيير">{{ old('justification', $changeOrder->justification) }}</textarea>
            </div>
        </div>
