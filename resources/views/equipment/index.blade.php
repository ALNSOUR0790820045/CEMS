        ...
        <form method="GET" action="{{ route('equipment.index') }}">
            <div class="filters-grid">
                <div class="filter-group">
                    ...
                    <input type="text" name="search" class="filter-input" placeholder="رقم المعدة، الاسم، الرقم التسلسلي..." value="{{ request('search') }}">
                </div>
                <div class="filter-group">
                    ...
                    <select name="status" class="filter-select">
                        ...
                        <option value="breakdown" {{ request('status') == 'breakdown' ? 'selected' : '' }}>عطل</option>
                    </select>
                </div>
                <div class="filter-group">
    ...
    @if($equipment->count() > 0)
        <div class="equipment-grid">
            @foreach($equipment as $item)
                <div class="equipment-card" onclick="window.location='{{ route('equipment.show', $item) }}'">
                    <div class="equipment-header">
                        <div>
                            <div class="equipment-number">{{ $item->equipment_number }}</div>
                            <h3 class="equipment-name">{{ $item->name }}</h3>
                            <div class="equipment-category">{{ $item->category->name }}</div>
                        </div>
                    </div>
                    ...
                    <div class="equipment-details">
                        ...
                        @if($item->currentProject)
                            <div class="detail-row">
                                ...
                                <span class="detail-value">{{ $item->currentProject->name }}</span>
                            </div>
                        @endif
                        <div class="detail-row">
                            <span class="detail-label">ساعات التشغيل:</span>
                            <span class="detail-value">{{ number_format($item->current_hours, 1) }} ساعة</span>
                        </div>
                    </div>
                    ...
                    <div class="equipment-footer">
                        ...
                        <div class="equipment-actions" onclick="event.stopPropagation()">
                            <a href="{{ route('equipment.edit', $item) }}" class="btn-icon" title="تعديل">
                                <i data-lucide="edit" style="width: 16px; height: 16px;"></i>
                            </a>
                            <a href="{{ route('equipment.show', $item) }}" class="btn-icon" title="عرض">
