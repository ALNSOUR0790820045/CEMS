...
<div style="padding: 40px; max-width: 800px; margin: 0 auto;">
    <h1 style="margin-bottom: 30px;">تعديل القسم</h1>
    
    <form method="POST" action="{{ route('departments.update', $department) }}" style="background: white; padding: 30px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="name" value="{{ old('name', $department->name) }}" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            @error('name')
            <span style="color: #ff3b30; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="name_en" value="{{ old('name_en', $department->name_en) }}" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            @error('name_en')
            <span style="color: #ff3b30; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" name="code" value="{{ old('code', $department->code) }}" required placeholder="مثال: HR, SALES" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            @error('code')
            <span style="color: #ff3b30; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="description" rows="4" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">{{ old('description', $department->description) }}</textarea>
            @error('description')
            <span style="color: #ff3b30; font-size: 0.85rem;">{{ $message }}</span>
            @enderror
        </div>
