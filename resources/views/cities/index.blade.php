...
<div style="padding: 40px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        ...
        <a href="{{ route('cities.create') }}" style="background: linear-gradient(135deg, #0071e3 0%, #00c4cc 100%); color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px;">
    ...
    <div style="background: white; padding: 20px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <form method="GET" action="{{ route('cities.index') }}" style="display: flex; gap: 15px; align-items: end;">
            <!-- Country Filter -->
            <div style="flex: 1;">
            ...
            <div style="flex: 1;">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="ابحث عن مدينة..." style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-family: 'Cairo', sans-serif;">
            </div>
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <a href="{{ route('cities.index') }}" style="background: #f5f5f7; color: #1d1d1f; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-family: 'Cairo', sans-serif; font-weight: 600; display: inline-flex; align-items: center;">
                    إعادة تعيين
                </a>
            </div>
        </form>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #f0f0f0;">
                    ...
                    <td style="padding: 15px; font-weight: 600;">{{ $city->name }}</td>
                    <td style="padding: 15px; color: #86868b;">{{ $city->name_en ?? '-' }}</td>
                    <td style="padding: 15px; color: #86868b;">{{ $city->country->name }}</td>
                    ...
                    <td style="padding: 15px; text-align: center;">
                        <div style="display: inline-flex; gap: 8px;">
                            <a href="{{ route('cities.edit', $city) }}" style="background: #0071e3; color: white; padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 0.85rem;">تعديل</a>
                            <form method="POST" action="{{ route('cities.destroy', $city) }}" style="display: inline;" onsubmit="return confirm('هل أنت متأكد من حذف هذه المدينة؟');">
                                @csrf
                                @method('DELETE')
