...
<div style="padding: 40px; max-width: 900px; margin: 0 auto;">
    <div style="margin-bottom: 30px;">
        <h1 style="font-size: 2rem; color: #1d1d1f; margin-bottom: 5px;">تعديل شرط الدفع</h1>
        <p style="color: #86868b;">تعديل بيانات شرط الدفع</p>
    </div>
    ...
    <form method="POST" action="{{ route('payment-terms.update', $paymentTerm) }}" style="background: white; padding: 40px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $paymentTerm->name) }}" required placeholder="نقدي، آجل 30 يوم"
            ...
            <div>
                ...
                <input type="text" name="name_en" value="{{ old('name_en', $paymentTerm->name_en) }}" placeholder="Cash, Net 30"
                    style="width: 100%; padding: 12px; border: 1px solid #d2d2d7; border-radius: 8px; font-size: 1rem; font-family: 'Cairo', sans-serif;">
            </div>
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="number" name="days" value="{{ old('days', $paymentTerm->days) }}" required min="0" 
                style="width: 100%; padding: 12px; border: 1px solid #d2d2d7; border-radius: 8px; font-size: 1rem; font-family: 'Cairo', sans-serif;">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="description" rows="4" placeholder="وصف شرط الدفع (اختياري)"
                style="width: 100%; padding: 12px; border: 1px solid #d2d2d7; border-radius: 8px; font-size: 1rem; font-family: 'Cairo', sans-serif; resize: vertical;">{{ old('description', $paymentTerm->description) }}</textarea>
        </div>
