...
<div style="padding: 40px; max-width: 1400px; margin: 0 auto;">
    <div style="margin-bottom: 30px;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="display: flex; gap: 10px;">
                <a href="{{ route('warehouses.edit', $warehouse) }}" style="background: #0071e3; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 8px;">
    ...
    <div style="background: white; padding: 30px; border-radius: 10px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
        <div style="display: flex; justify-content: space-between; align-items: start;">
            <div>
                <h2 style="margin: 0 0 10px 0; font-size: 1.5rem; font-weight: 700;">{{ $warehouse->name }}</h2>
                @if($warehouse->name_en)
                <p style="margin: 0 0 10px 0; color: #666;">{{ $warehouse->name_en }}</p>
                @endif
                <p style="margin: 0; color: #0071e3; font-weight: 600; font-size: 1.1rem;">{{ $warehouse->code }}</p>
            </div>
            <div style="display: flex; gap: 10px;">
        ...
        <div style="background: white; padding: 30px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
                <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                    ...
                    <div style="font-weight: 600;">{{ $warehouse->company?->name ?? 'غير محدد' }}</div>
                </div>

                <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                    <div style="font-size: 0.85rem; color: #666; margin-bottom: 5px;">الفرع</div>
                    <div style="font-weight: 600;">{{ $warehouse->branch?->name ?? 'غير محدد' }}</div>
                </div>

                <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                    <div style="font-size: 0.85rem; color: #666; margin-bottom: 5px;">المدير</div>
                    <div style="font-weight: 600;">{{ $warehouse->manager?->name ?? 'غير محدد' }}</div>
                </div>

                <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                    <div style="font-size: 0.85rem; color: #666; margin-bottom: 5px;">الهاتف</div>
                    <div style="font-weight: 600;">{{ $warehouse->phone ?? 'غير محدد' }}</div>
                </div>

                <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                    <div style="font-size: 0.85rem; color: #666; margin-bottom: 5px;">المدينة</div>
                    <div style="font-weight: 600;">{{ $warehouse->city?->name ?? 'غير محدد' }}</div>
                </div>

                <div style="padding: 15px; background: #f9f9f9; border-radius: 8px;">
                    <div style="font-size: 0.85rem; color: #666; margin-bottom: 5px;">تاريخ الإضافة</div>
                    <div style="font-weight: 600;">{{ $warehouse->created_at->format('Y-m-d H:i') }}</div>
                </div>
            </div>
            ...
            <div style="margin-top: 20px; padding: 15px; background: #f9f9f9; border-radius: 8px;">
                ...
                <div style="font-weight: 600;">{{ $warehouse->address }}</div>
            </div>
            @endif
        </div>
    </div>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #e5e5e7;">
                        ...
                        <td style="padding: 12px;">{{ $movement->notes ?? '-' }}</td>
                        <td style="padding: 12px;">{{ $movement->creator?->name ?? '-' }}</td>
