...
<div style="padding: 40px; max-width: 900px; margin: 0 auto;">
    ...
    <form method="POST" action="{{ route('warehouses.store') }}" style="background: white; padding: 30px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
        @csrf

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            <div>
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">كود المستودع *</label>
                <input type="text" name="code" value="{{ old('code') }}" required placeholder="WH001" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-family: 'Cairo', sans-serif;">
            </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="address" rows="3" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-family: 'Cairo', sans-serif; resize: vertical;">{{ old('address') }}</textarea>
        </div>

        <div style="display: flex; gap: 30px; margin-bottom: 30px; padding: 15px; background: #f5f5f7; border-radius: 8px;">
            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                <input type="checkbox" name="is_main" value="1" {{ old('is_main') ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
