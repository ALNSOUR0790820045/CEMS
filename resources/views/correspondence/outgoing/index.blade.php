...
<div style="padding: 40px; max-width: 1400px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <h1 style="margin: 0;">المراسلات الصادرة</h1>
        <a href="{{ route('correspondence.create') }}?type=outgoing" style="background: #0071e3; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600;">
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); overflow: hidden;">
        @if($correspondences->isEmpty())
            <div style="padding: 60px; text-align: center; color: #86868b;">
                <i data-lucide="send" style="width: 64px; height: 64px; margin-bottom: 20px; opacity: 0.3;"></i>
                <p style="font-size: 1.1rem; margin: 0;">لا توجد مراسلات صادرة</p>
            </div>
        @else
            <table style="width: 100%; border-collapse: collapse;">
                <thead style="background: #f5f5f7;">
                    <tr>
                        ...
                        <th style="padding: 15px; text-align: right; font-weight: 600; color: #1d1d1f;">إلى</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid #f5f5f7;">
                        ...
                        <td style="padding: 15px;">{{ $correspondence->to_entity }}</td>
                        <td style="padding: 15px; color: #86868b; font-size: 0.9rem;">
                            {{ $correspondence->sent_date?->format('Y-m-d') ?? $correspondence->document_date->format('Y-m-d') }}
                        </td>
                        <td style="padding: 15px;">
                            @php
                                $statusColors = [
