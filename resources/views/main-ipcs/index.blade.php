...
<div style="max-width: 1400px; margin: 0 auto;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        ...
        <a href="{{ route('main-ipcs.create') }}" style="background: var(--accent); color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; display: flex; align-items: center; gap: 8px;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="font-size: 1.8rem; font-weight: 700;">{{ $statistics['total_count'] }}</div>
            <div style="color: #86868b; font-size: 0.8rem; margin-top: 5px;">{{ number_format($statistics['total_value'], 2) }} ر.س</div>
        </div>
        ...
        <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="font-size: 1.8rem; font-weight: 700;">{{ $statistics['pending_count'] }}</div>
        ...
        <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="font-size: 1.8rem; font-weight: 700;">{{ $statistics['approved_count'] }}</div>
        ...
        <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="font-size: 1.8rem; font-weight: 700;">{{ $statistics['paid_count'] }}</div>
            <div style="color: #86868b; font-size: 0.8rem; margin-top: 5px;">{{ number_format($statistics['paid_value'], 2) }} ر.س</div>
        </div>
    </div>

    <!-- Filters -->
    <div style="background: white; padding: 20px; border-radius: 12px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
        <form method="GET" action="{{ route('main-ipcs.index') }}" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; align-items: end;">
            <div>
            ...
            <div>
                ...
                <select name="status" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-family: 'Cairo', sans-serif;">
                    ...
                    <option value="pending_pm" {{ request('status') == 'pending_pm' ? 'selected' : '' }}>معلق - مدير المشروع</option>
                    <option value="pending_technical" {{ request('status') == 'pending_technical' ? 'selected' : '' }}>معلق - المدير الفني</option>
                    <option value="pending_consultant" {{ request('status') == 'pending_consultant' ? 'selected' : '' }}>معلق - الاستشاري</option>
                    <option value="pending_client" {{ request('status') == 'pending_client' ? 'selected' : '' }}>معلق - العميل</option>
                    <option value="pending_finance" {{ request('status') == 'pending_finance' ? 'selected' : '' }}>معلق - المالية</option>
                    <option value="approved_for_payment" {{ request('status') == 'approved_for_payment' ? 'selected' : '' }}>جاهز للدفع</option>
            ...
            <div>
                ...
                <input type="date" name="date_from" value="{{ request('date_from') }}" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-family: 'Cairo', sans-serif;">
            </div>

            <div>
                <label style="display: block; margin-bottom: 5px; font-size: 0.85rem; color: #86868b;">إلى تاريخ</label>
                <input type="date" name="date_to" value="{{ request('date_to') }}" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-family: 'Cairo', sans-serif;">
            </div>
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <a href="{{ route('main-ipcs.index') }}" style="background: #f5f5f7; color: #1d1d1f; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: 600; display: flex; align-items: center;">
                    إعادة تعيين
                </a>
            </div>
        </form>
                ...
                @forelse($ipcs as $ipc)
                    <tr style="border-bottom: 1px solid #e5e5e7;">
                        <td style="padding: 15px;">
                            <div style="font-weight: 600; color: var(--accent);">{{ $ipc->ipc_number }}</div>
                            <div style="font-size: 0.75rem; color: #86868b;">تسلسل: {{ $ipc->ipc_sequence }}</div>
                        </td>
                        <td style="padding: 15px;">
                            <div style="font-weight: 500;">{{ $ipc->project->name }}</div>
                        </td>
                        <td style="padding: 15px;">
                            <div style="font-size: 0.85rem;">{{ $ipc->period_from->format('Y/m/d') }}</div>
                            <div style="font-size: 0.75rem; color: #86868b;">إلى {{ $ipc->period_to->format('Y/m/d') }}</div>
                        </td>
                        <td style="padding: 15px;">
                            <div style="font-weight: 600;">{{ number_format($ipc->current_cumulative, 2) }} ر.س</div>
                        </td>
                        <td style="padding: 15px;">
                            <div style="font-weight: 600; color: #34c759;">{{ number_format($ipc->net_payable, 2) }} ر.س</div>
                        </td>
                        <td style="padding: 15px;">
                            <span style="padding: 6px 12px; border-radius: 6px; font-size: 0.75rem; font-weight: 600; 
                                background: rgba({{ $ipc->status_badge['color'] === 'green' ? '52, 199, 89' : ($ipc->status_badge['color'] === 'blue' ? '0, 113, 227' : ($ipc->status_badge['color'] === 'yellow' ? '255, 204, 0' : ($ipc->status_badge['color'] === 'red' ? '255, 59, 48' : '134, 134, 139'))) }}, 0.1);
                                color: {{ $ipc->status_badge['color'] === 'green' ? '#34c759' : ($ipc->status_badge['color'] === 'blue' ? '#0071e3' : ($ipc->status_badge['color'] === 'yellow' ? '#ffcc00' : ($ipc->status_badge['color'] === 'red' ? '#ff3b30' : '#86868b'))) }};">
                        ...
                        <td style="padding: 15px;">
                            <div style="display: flex; flex-direction: column; gap: 5px;">
                                <div style="width: 100%; height: 6px; background: #e5e5e7; border-radius: 3px; overflow: hidden;">
                                    <div style="width: {{ $ipc->approval_progress }}%; height: 100%; background: linear-gradient(90deg, #0071e3, #00c4cc); transition: width 0.3s;"></div>
                                </div>
                                <div style="font-size: 0.75rem; color: #86868b;">{{ $ipc->approval_progress }}% مكتمل</div>
                            </div>
                        </td>
                        <td style="padding: 15px; text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <a href="{{ route('main-ipcs.show', $ipc) }}" style="padding: 6px 12px; background: rgba(0, 113, 227, 0.1); color: var(--accent); border-radius: 6px; text-decoration: none; font-size: 0.85rem; font-weight: 600;">
                                    عرض
                                </a>
                                @if(in_array($ipc->status, ['pending_pm', 'pending_technical', 'pending_consultant', 'pending_client', 'pending_finance']))
                                    <a href="{{ route('main-ipcs.approve', $ipc) }}" style="padding: 6px 12px; background: rgba(52, 199, 89, 0.1); color: #34c759; border-radius: 6px; text-decoration: none; font-size: 0.85rem; font-weight: 600;">
                                        مراجعة
                                    </a>
                                @endif
                                @if($ipc->status === 'approved_for_payment')
                                    <a href="{{ route('main-ipcs.payment', $ipc) }}" style="padding: 6px 12px; background: rgba(255, 149, 0, 0.1); color: #ff9500; border-radius: 6px; text-decoration: none; font-size: 0.85rem; font-weight: 600;">
                                        دفع
                                    </a>
                                @endif
                            </div>
