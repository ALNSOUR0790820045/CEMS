...
<div style="padding: 40px; max-width: 1400px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        ...
        <a href="{{ route('claims.create') }}" style="background: #0071e3; color: white; padding: 12px 30px; border-radius: 8px; text-decoration: none; font-weight: 600;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #dee2e6;">
                    <td style="padding: 15px;">{{ $claim->claim_number }}</td>
                    <td style="padding: 15px;">{{ $claim->title }}</td>
                    <td style="padding: 15px;">{{ $claim->project->name ?? '-' }}</td>
                    <td style="padding: 15px;">
                        <span style="padding: 4px 12px; background: #e3f2fd; color: #1976d2; border-radius: 12px; font-size: 0.85rem;">
                            {{ $claim->type_label }}
                        </span>
                    </td>
                    <td style="padding: 15px;">{{ number_format($claim->claimed_amount, 2) }} {{ $claim->currency }}</td>
                    <td style="padding: 15px;">
                    ...
                    <td style="padding: 15px;">
                        <a href="{{ route('claims.show', $claim) }}" style="color: #0071e3; text-decoration: none; margin-left: 10px;">
                            <i data-lucide="eye" style="width: 18px; height: 18px;"></i>
                        </a>
                        <a href="{{ route('claims.edit', $claim) }}" style="color: #0071e3; text-decoration: none; margin-left: 10px;">
                            <i data-lucide="edit" style="width: 18px; height: 18px;"></i>
                        </a>
                        <form method="POST" action="{{ route('claims.destroy', $claim) }}" style="display: inline;" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                            @csrf
                            @method('DELETE')
