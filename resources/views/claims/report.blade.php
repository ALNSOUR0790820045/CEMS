...
<head>
    ...
    <title>تقرير المطالبة - {{ $claim->claim_number }}</title>
    <style>
        @page {
            margin: 2cm;
        }
...
<body>
    ...
    <div class="section">
        ...
        <div class="info-grid">
            ...
            <div class="info-row">
                ...
                <div class="info-value">{{ $claim->project->name ?? '-' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">العقد</div>
                <div class="info-value">{{ $claim->contract->title ?? '-' }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">النوع</div>
                <div class="info-value">{{ $claim->type_label }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">السبب</div>
                <div class="info-value">{{ $claim->cause_label }}</div>
            </div>
            <div class="info-row">
                <div class="info-label">الحالة</div>
                <div class="info-value">{{ $claim->status_label }}</div>
            </div>
            <div class="info-row">
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ $claim->claimed_days }}</td>
                </tr>
                <tr>
                    <td><strong>المقيم</strong></td>
                    <td>{{ number_format($claim->assessed_amount, 2) }}</td>
                    <td>{{ $claim->assessed_days }}</td>
                </tr>
                <tr>
                    <td><strong>المعتمد</strong></td>
                    <td>{{ number_format($claim->approved_amount, 2) }}</td>
    ...
    <div class="section">
        ...
        <div class="info-grid">
            <div class="info-row">
                ...
                <div class="info-value">{{ $claim->event_start_date->format('Y-m-d') }}</div>
            </div>
            @if($claim->event_end_date)
            <div class="info-row">
                <div class="info-label">نهاية الحدث</div>
                <div class="info-value">{{ $claim->event_end_date->format('Y-m-d') }}</div>
            </div>
            @endif
            <div class="info-row">
                <div class="info-label">تاريخ الإشعار</div>
                <div class="info-value">{{ $claim->notice_date->format('Y-m-d') }}</div>
            </div>
            @if($claim->submission_date)
            <div class="info-row">
                <div class="info-label">تاريخ التقديم</div>
                <div class="info-value">{{ $claim->submission_date->format('Y-m-d') }}</div>
            </div>
            @endif
            @if($claim->response_due_date)
            <div class="info-row">
                <div class="info-label">تاريخ استحقاق الرد</div>
                <div class="info-value">{{ $claim->response_due_date->format('Y-m-d') }}</div>
            </div>
            @endif
            @if($claim->response_date)
            <div class="info-row">
                <div class="info-label">تاريخ الرد</div>
                <div class="info-value">{{ $claim->response_date->format('Y-m-d') }}</div>
            </div>
            @endif
            @if($claim->resolution_date)
            <div class="info-row">
                <div class="info-label">تاريخ التسوية</div>
                <div class="info-value">{{ $claim->resolution_date->format('Y-m-d') }}</div>
            </div>
            @endif
        </div>
    </div>

    @if($claim->timeline->count() > 0)
    <div class="section">
        <h2>Timeline</h2>
        ...
        <div class="timeline-item">
            ...
            @if($event->notes)
            <p style="margin-top: 5px;">{{ $event->notes }}</p>
            @endif
        </div>
        @endforeach
    </div>
    ...
    @if($claim->client_response)
    <div class="section">
        <h2>رد العميل</h2>
        <p>{{ $claim->client_response }}</p>
    </div>
    @endif
    ...
    <div class="section">
        ...
        <p>{{ $claim->resolution_notes }}</p>
    </div>
    @endif

    @if($claim->lessons_learned)
    <div class="section">
        <h2>الدروس المستفادة</h2>
        <p>{{ $claim->lessons_learned }}</p>
    </div>
    @endif
    ...
    <div class="section">
        ...
        <div class="info-grid">
            <div class="info-row">
                ...
                <div class="info-value">{{ $claim->preparedBy->name }}</div>
            </div>
            @if($claim->reviewedBy)
            <div class="info-row">
                <div class="info-label">راجعه</div>
                <div class="info-value">{{ $claim->reviewedBy->name }}</div>
            </div>
            @endif
        </div>
    </div>
    ...
    <div style="margin-top: 50px; padding-top: 20px; border-top: 2px solid #ddd; text-align: center; color: #666; font-size: 0.9em;">
        <p>تم إنشاء هذا التقرير في {{ now()->format('Y-m-d H:i') }}</p>
