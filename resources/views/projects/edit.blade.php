...
<div style="padding: 40px; max-width: 1000px; margin: 0 auto;">
    ...
    <form method="POST" action="{{ route('projects.update', $project) }}" style="background: white; padding: 30px; border-radius: 10px;">
        ...
        <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px;">
            ...
            <p style="margin: 0; font-weight: 600; font-size: 16px;">{{ $project->project_number }}</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div>
                ...
                <select name="type" required 
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
                    <option value="building" {{ old('type', $project->type) == 'building' ? 'selected' : '' }}>مباني</option>
                    <option value="infrastructure" {{ old('type', $project->type) == 'infrastructure' ? 'selected' : '' }}>بنية تحتية</option>
                    <option value="industrial" {{ old('type', $project->type) == 'industrial' ? 'selected' : '' }}>صناعي</option>
                    <option value="maintenance" {{ old('type', $project->type) == 'maintenance' ? 'selected' : '' }}>صيانة</option>
                    <option value="fit_out" {{ old('type', $project->type) == 'fit_out' ? 'selected' : '' }}>تشطيبات</option>
                    <option value="other" {{ old('type', $project->type) == 'other' ? 'selected' : '' }}>أخرى</option>
                </select>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 30px;">
            <div>
                ...
                <input type="date" name="commencement_date" value="{{ old('commencement_date', $project->commencement_date->format('Y-m-d')) }}" required 
                       style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            </div>
            
            <div>
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">تاريخ الانتهاء المخطط *</label>
                <input type="date" name="original_completion_date" value="{{ old('original_completion_date', $project->original_completion_date->format('Y-m-d')) }}" required 
                       style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            </div>
        ...
        <div style="margin-bottom: 30px;">
            ...
            <input type="number" name="original_duration_days" value="{{ old('original_duration_days', $project->original_duration_days) }}" required min="1" 
                   style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 30px;">
            <div>
                ...
                <input type="number" name="original_contract_value" value="{{ old('original_contract_value', $project->original_contract_value) }}" required min="0" step="0.01" 
                       style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            </div>
            
            <div>
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">قيمة العقد المعدلة *</label>
                <input type="number" name="revised_contract_value" value="{{ old('revised_contract_value', $project->revised_contract_value) }}" required min="0" step="0.01" 
                       style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px; font-family: 'Cairo', sans-serif;">
            </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 30px;">
            <div>
                ...
                <select name="status" required 
                    ...
                    <option value="mobilization" {{ old('status', $project->status) == 'mobilization' ? 'selected' : '' }}>تجهيز الموقع</option>
                    ...
                    <option value="suspended" {{ old('status', $project->status) == 'suspended' ? 'selected' : '' }}>معلق</option>
                    <option value="completed" {{ old('status', $project->status) == 'completed' ? 'selected' : '' }}>منتهي</option>
                    <option value="handed_over" {{ old('status', $project->status) == 'handed_over' ? 'selected' : '' }}>تم التسليم</option>
                    <option value="closed" {{ old('status', $project->status) == 'closed' ? 'selected' : '' }}>مغلق</option>
                </select>
            </div>
