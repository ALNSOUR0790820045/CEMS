        ...
        <table style="width: 100%; border-collapse:  collapse; min-width: 1200px;">
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #f5f5f7;">
                    <td style="padding: 15px; color: #0071e3; font-weight: 600; white-space: nowrap;">{{ $project->project_code ??  $project->project_number }}</td>
                    <td style="padding: 15px; font-weight: 600;">{{ $project->name }}</td>
                    <td style="padding: 15px;">{{ $project->client->name }}</td>
                    <td style="padding: 15px;">
                        @php $status = $project->project_status ??  $project->status; @endphp
                        @if($status == 'tendering')
                        ...
                        @elseif(in_array($status, ['mobilization', 'not_started']))
                        ...
                        @elseif(in_array($status, ['execution', 'in_progress']))
                    ...
                    <td style="padding: 15px; white-space: nowrap;">
                        {{ number_format($project->contract_value ??  $project->revised_contract_value, 2) }} 
                        {{ $project->currency->code ??  $project->currency }}
                    </td>
                    <td style="padding: 15px; white-space: nowrap;">
                        {{ ($project->contract_start_date ?? $project->commencement_date)->format('Y-m-d') }}
                    </td>
                    <td style="padding:  15px; white-space:  nowrap;">
                        {{ ($project->contract_end_date ??  $project->original_completion_date)->format('Y-m-d') }}
                    </td>
                    <td style="padding: 15px;">{{ $project->projectManager->name ??  'غير محدد' }}</td>
