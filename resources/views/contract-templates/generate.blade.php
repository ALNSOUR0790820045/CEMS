...
<div class="page-header">
    ...
    <h1 class="page-title">إنشاء عقد من قالب: {{ $contractTemplate->name }}</h1>
    <p style="color: #86868b; margin-top: 8px;">قم بتعبئة البيانات المطلوبة لإنشاء العقد</p>
</div>

<form action="{{ route('contract-templates.store-generated') }}" method="POST" class="form-container">
    @csrf
    ...
    <!-- المتغيرات -->
    @if($contractTemplate->variables->count() > 0)
    <div class="form-section">
        <h3 class="section-title">
        ...
        <div class="form-grid">
            @foreach($contractTemplate->variables as $variable)
            <div class="form-group">
                ...
                @if($variable->data_type === 'text')
                    <input type="text" 
                           name="filled_data[{{ $variable->variable_key }}]" 
                           class="form-input"
                           @if($variable->is_required) required @endif
                           value="{{ $variable->default_value }}">
                @elseif($variable->data_type === 'number')
                    <input type="number" 
                           name="filled_data[{{ $variable->variable_key }}]" 
                           class="form-input"
                           @if($variable->is_required) required @endif
                ...
                @elseif($variable->data_type === 'date')
                    <input type="date" 
                           ...
                           @if($variable->is_required) required @endif
                           value="{{ $variable->default_value }}">
                @elseif($variable->data_type === 'currency')
                    <input type="number" 
                           step="0.01"
                           name="filled_data[{{ $variable->variable_key }}]" 
                           class="form-input"
    ...
    <div class="form-actions">
        ...
        <a href="{{ route('contract-templates.show', $contractTemplate->id) }}" class="btn btn-secondary">
