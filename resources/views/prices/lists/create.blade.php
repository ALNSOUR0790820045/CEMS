...
<div style="padding: 20px; max-width: 1200px; margin: 0 auto;">
    <div style="margin-bottom: 20px;">
        ...
        <h1 style="font-size: 28px; font-weight: 600; margin: 10px 0;">إنشاء قائمة أسعار جديدة</h1>
    </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
                ...
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="labor" {{ old('type') == 'labor' ? 'selected' : '' }}>عمالة</option>
                        <option value="equipment" {{ old('type') == 'equipment' ? 'selected' : '' }}>معدات</option>
                        <option value="subcontract" {{ old('type') == 'subcontract' ? 'selected' : '' }}>مقاولين</option>
                        <option value="composite" {{ old('type') == 'composite' ? 'selected' : '' }}>مركب</option>
                    </select>
                    @error('type')
                        <span style="color: #dc3545; font-size: 13px;">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <select name="source" required
                        ...
                        <option value="internal" {{ old('source') == 'internal' ? 'selected' : '' }}>داخلي</option>
                        <option value="ministry" {{ old('source') == 'ministry' ? 'selected' : '' }}>وزارة الأشغال</option>
                        <option value="syndicate" {{ old('source') == 'syndicate' ? 'selected' : '' }}>النقابة</option>
                        <option value="market" {{ old('source') == 'market' ? 'selected' : '' }}>السوق</option>
                        <option value="vendor" {{ old('source') == 'vendor' ? 'selected' : '' }}>مورد</option>
                    </select>
                    @error('source')
                        <span style="color: #dc3545; font-size: 13px;">{{ $message }}</span>
                    @enderror
